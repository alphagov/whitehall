...
class WorldLocationIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "when updating, makes the correct calls to publishing api" do
    Sidekiq::Testing.inline! do
      world_location_news_wo_translations = build(:world_location_news)
      world_location = create(:world_location, world_location_news: world_location_news_wo_translations, name: "germany")
      visit edit_admin_world_location_news_path(world_location_news_wo_translations)
      new_mission_statement = "a different mission"
      fill_in "world_location_news_mission_statement", with: new_mission_statement

      Services.publishing_api.expects(:put_content).once.with(world_location_news_wo_translations.content_id, put_content_hash_containing("en", world_location_news_wo_translations.title, new_mission_statement))
  ...
  test "when updating a world location news without translations, makes the correct call to search api" do
    Sidekiq::Testing.inline! do
      world_location_news_wo_translations = build(:world_location_news)
      create(:world_location, world_location_news: world_location_news_wo_translations, name: "germany", active: true)
      visit edit_admin_world_location_news_path(world_location_news_wo_translations)
      new_mission_statement = "a different mission"
      fill_in "world_location_news_mission_statement", with: new_mission_statement

      Whitehall::FakeRummageableIndex.any_instance.expects(:add).twice.with(search_index_add_parameters(world_location_news_wo_translations))
  ...
  test "when updating an inactive location, does not make any calls to search api" do
    Sidekiq::Testing.inline! do
      world_location_news_wo_translations = build(:world_location_news)
      create(:world_location, world_location_news: world_location_news_wo_translations, name: "germany", active: false)
      visit edit_admin_world_location_news_path(world_location_news_wo_translations)
      new_mission_statement = "a different mission"
      fill_in "world_location_news_mission_statement", with: new_mission_statement
