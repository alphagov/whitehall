...
class WorldLocationIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "when updating, makes the correct calls to search api and publishing api" do
    Sidekiq::Testing.inline! do
      world_location_wo_translations = create(:world_location, news_page_content_id: "id-123")
      visit edit_admin_world_location_path(world_location_wo_translations)
      new_mission_statement = "a different mission"
      fill_in "world_location_mission_statement", with: new_mission_statement

      p = PublishingApi::WorldLocationNewsPresenter.new(world_location_wo_translations)

      Services.publishing_api.expects(:put_content).once.with("id-123", put_content_hash_containing("en",  world_location_wo_translations.title, new_mission_statement))
      Services.publishing_api.expects(:put_content).once.with(world_location_wo_translations.content_id, has_entries(document_type: "world_location"))
      Whitehall::FakeRummageableIndex.any_instance.expects(:add).with(p.content_for_rummager("id-123"))
  ...
  test "when updating an inactive location, does not make any calls to search api" do
    Sidekiq::Testing.inline! do
      world_location_wo_translations = create(:world_location, news_page_content_id: "id-123", active: false)
      visit edit_admin_world_location_path(world_location_wo_translations)
      new_mission_statement = "a different mission"
      fill_in "world_location_mission_statement", with: new_mission_statement
