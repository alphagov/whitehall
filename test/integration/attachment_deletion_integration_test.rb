...
class AttachmentDeletionIntegrationTest < ActionDispatch::IntegrationTest
  ...
  describe "attachment deletion" do
    ...
    context "unpublishing a published document" do
      let(:managing_editor) { create(:managing_editor) }
      let(:attachable) { create(:published_news_article, :with_file_attachment) }
      let(:attachment) { attachable.attachments.first }
      let(:original_asset) { attachment.attachment_data.assets.first.asset_manager_id }
      let(:thumbnail_asset) { attachment.attachment_data.assets.second.asset_manager_id }
      ...
      before do
        ...
        setup_publishing_api_for(attachable)
        stub_publishing_api_has_linkables([], document_type: "topic")
        stub_publishing_api_expanded_links_with_taxons(attachable.content_id, [])

        stub_asset(original_asset)
        stub_asset(thumbnail_asset)
      end

      it "deletes the corresponding assets in Asset Manager" do
        visit admin_news_article_path(attachable)
        click_link "Withdraw or unpublish"
        page.choose("Unpublish: published in error")
        page.find_all('button[type="submit"]', text: 'Unpublish', visible: true).first.click
