...
class ContactsExtractorTest < ActiveSupport::TestCase
  ...
  test "extracted_contact_ids method returns all IDs extracted from the input, regardless of whether Contact exists or not" do
    input = "[Contact:99999999]"

    embedded_contacts = Govspeak::ContactsExtractor.new(input).extracted_contact_ids
    assert_equal [99_999_999], embedded_contacts
  end
end
