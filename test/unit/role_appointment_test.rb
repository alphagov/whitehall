...
class RoleAppointmentTest < ActiveSupport::TestCase
  ...
  test "should link the Person to the MinisterialRoles they currently hold" do
    ...
    assert_equal [new_role, other_new_role], alice.roles
  end
  ...
  test "should make appointments historical when a new Person is appointed to a MinisterialRole" do
    ...
    assert_equal alice, role.person, "the minister should be alice"
    assert_equal [role], alice.roles, "alice should be the minister"
    assert_equal [], bob.roles, "bob should have no roles"
    ...
    assert_equal [], alice.roles, "alice should no longer be the minister"
    assert_equal bob, role.person, "the minister should be bob"
    assert_equal [role], bob.roles, "bob should be the minister"
  end
