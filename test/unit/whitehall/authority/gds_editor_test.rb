...
class GDSEditorTest < ActiveSupport::TestCase
  def gds_editor
    OpenStruct.new(gds_editor?: true)
  end
  ...
  test 'can make a fact check request for a document' do
    assert enforcer_for(gds_editor, normal_edition).can?(:request_fact_check)
  end
  ...
  test 'can review a submitted edition' do
    assert enforcer_for(gds_editor, submitted_edition).can?(:approve)
  end

  test 'can publish a submitted edition' do
    assert enforcer_for(gds_editor, submitted_edition).can?(:publish)
  end

  test 'can reject a submitted edition' do
    assert enforcer_for(gds_editor, submitted_edition).can?(:reject)
  end

  test 'can force publish a edition' do
    assert enforcer_for(gds_editor, submitted_edition).can?(:force_publish)
  end
  ...
  test 'can clear the "not reviewed" flag on editions they didn\'t force publish' do
    assert enforcer_for(gds_editor, force_published_edition(gds_editor)).can?(:review)
  end
