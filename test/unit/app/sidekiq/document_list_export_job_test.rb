...
class DocumentListExportJobTest < ActiveSupport::TestCase
  ...
  test "uploads csv to s3" do
    ...
    @job.stubs(:create_filter).returns(stub(page_title: title))
    @job.stubs(generate_csv: csv)
    @job.stubs(:send_mail)
