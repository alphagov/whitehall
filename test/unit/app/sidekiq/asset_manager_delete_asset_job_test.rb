...
class AssetManagerDeleteAssetJobTest < ActiveSupport::TestCase
  setup do
    ...
    @job = AssetManagerDeleteAssetJob.new
  end

  test "it calls AssetManager::AssetDeleter" do
    AssetManager::AssetDeleter.expects(:call).with("any-asset-id")

    @job.perform("any-asset-id")
  end
  ...
  test "it raises an error if the asset deletion fails for an unknown reason" do
    ...
    assert_raises(GdsApi::HTTPServerError) { @job.perform(@asset_manager_id) }
  end

  test "it deletes the Asset from Asset table" do
    AssetManager::AssetDeleter.stubs(:call)

    @job.perform(@asset_manager_id)
