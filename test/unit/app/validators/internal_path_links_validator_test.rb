...
class InternalPathLinksValidatorTest < ActiveSupport::TestCase
  ...
  test "should identify internal admin links" do
    assert InternalPathLinksValidator.is_internal_admin_link?([Whitehall.router_prefix, "admin", "test"].join("/"))
    assert_not InternalPathLinksValidator.is_internal_admin_link?("http://www.google.com/")
    assert_not InternalPathLinksValidator.is_internal_admin_link?(nil)
  end
end
