...
class EditionUnpublisherTest < ActiveSupport::TestCase
  test "#perform! with a published edition changes the edition's state to removed and saves the unpublishing details" do
  ...
  test "other edition states cannot be unpublished" do
    (Edition.available_states - %i[published]).each do |state|
