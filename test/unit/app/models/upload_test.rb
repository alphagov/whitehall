...
class UploadTest < ActiveSupport::TestCase
  ...
  test "builds Attachment instances for an array of files" do
    ...
    assert_equal 2, upload.attachments.size
    assert_equal "simple.pdf", upload.attachments[0].filename
    assert_equal "whitepaper.pdf", upload.attachments[1].filename
  end
  ...
  test "loads attachments from the edition if filenames match and adds `new_filename`" do
    ...
    upload = Upload.new(edition)
    upload.build_attachments_from_files(files)
    assert upload.attachments.first.new_record?, "Attachment should be new record"
    assert_not upload.attachments.last.new_record?, "Attachment shouldn't be new record"
    assert_equal "#{existing.attachment_data.filename_without_extension}_1.#{existing.file_extension}", upload.attachments.last.attachment_data.new_filename
  end
  ...
  test "always builds new AttachmentData instances from array of files" do
    ...
    upload = Upload.new(edition)
    upload.build_attachments_from_files(files)
    assert(upload.attachments.all? { |a| a.attachment_data.new_record? })
  end
  ...
  test "sets replaced_by on existing AttachmentData when file re-attached" do
    ...
    upload = Upload.new(edition)
    upload.build_attachments_from_files(files)
    new_attachment_data = upload.attachments.last.attachment_data
  ...
  test "sets replaced_by on existing AttachmentData when file re-attached from attachments_attributes" do
    ...
    upload = Upload.new(edition)
    upload.build_attachments_from_params(params)
    upload.save_attachments
    new_attachment_data = upload.attachments.first.attachment_data
    assert_equal new_attachment_data, existing.attachment_data.reload.replaced_by
  end
  ...
  test "#save_attachments saves new attachments to the end of the edition's existing attachments" do
    ...
    assert_difference("edition.attachments.count", 2) do
      assert upload.save_attachments, "should return true"
  ...
  test "#save_attachments does not save any attachments if one is invalid" do
    edition = create(:detailed_guide)
    upload = Upload.new(edition)
    upload.build_attachments_from_params(invalid_new_attachments_params)
    assert_no_difference("edition.attachments.count") do
      assert_not upload.save_attachments, "should return false"
    end
  end
