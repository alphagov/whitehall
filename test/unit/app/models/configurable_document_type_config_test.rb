...
class ConfigurableDocumentTypeConfigTest < ActiveSupport::TestCase
  test "it builds a valid config object" do
    config = ConfigurableDocumentTypes::GovernmentResponse = ConfigurableDocumentTypeConfig.new.build do
    ...
    assert config["forms"]["documents"]["fields"].size.positive?
    assert config["settings"]["configurable_document_group"]
  end
end
