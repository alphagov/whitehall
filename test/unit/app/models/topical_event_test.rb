...
class TopicalEventTest < ActiveSupport::TestCase
  ...
  test "#latest should return specified number of associated published editions in reverse chronological order" do
    ...
    expected_order = [
      ...
      create(:published_standard_edition, topical_events: [topical_event], first_published_at: 1.week.ago),
      create(:published_publication, topical_events: [topical_event], first_published_at: 2.weeks.ago),
      create(:published_standard_edition, topical_events: [topical_event], first_published_at: 3.weeks.ago),
  ...
  test "an unfeatured edition is not featured" do
    topical_event = create(:topical_event)
    edition = build(:publication)
    assert_not topical_event.featured?(edition)
  end

  test "a featured edition is featured" do
    topical_event = create(:topical_event)
    edition = create(:published_publication)
    image = create(:topical_event_featuring_image_data)
    topical_event.feature(edition_id: edition.id, alt_text: "A thing", image:)
    featuring = topical_event.featuring_of(edition)
    assert featuring
    assert_equal 0, featuring.ordering
    assert topical_event.featured?(edition)
  end

  test "a featured edition is no longer featured when it is superseded" do
    topical_event = create(:topical_event)
    ...
    featuring = topical_event.reload.featuring_of(edition)
  ...
  test "#featured_editions returns featured editions by ordering" do
    ...
    beta = topical_event.feature(edition_id: create(:published_publication, title: "Beta").id, ordering: 2, alt_text: "A thing", image: create(:topical_event_featuring_image_data))
    gamma = topical_event.feature(edition_id: create(:published_publication, title: "Gamma").id, ordering: 3, alt_text: "A thing", image: create(:topical_event_featuring_image_data))
    delta = topical_event.feature(edition_id: create(:published_publication, title: "Delta").id, ordering: 0, alt_text: "A thing", image: create(:topical_event_featuring_image_data))
  ...
  test "#featured_editions includes the newly published version of a featured edition, but not the original" do
    topical_event = create(:topical_event)
    old_version = topical_event.feature(edition_id: create(:published_publication, title: "Gamma").id, ordering: 3, alt_text: "A thing", image: create(:topical_event_featuring_image_data)).edition
  ...
  test "#next_ordering gives the next value when there are existing features" do
    ...
    edition_1 = create(:published_publication)
    image_1 = create(:topical_event_featuring_image_data)
    topical_event.feature(edition_id: edition_1.id, alt_text: "A thing", image: image_1)

    edition_2 = create(:published_publication)
    image_2 = create(:topical_event_featuring_image_data)
    topical_event.feature(edition_id: edition_2.id, alt_text: "A thing", image: image_2)
  ...
  test "#next_ordering gives the next value when there are existing features that have been reordered" do
    ...
    edition_1 = create(:published_publication)
    image_1 = create(:topical_event_featuring_image_data)
    topical_event.feature(edition_id: edition_1.id, alt_text: "A thing", image: image_1, ordering: 1)

    edition_2 = create(:published_publication)
    image_2 = create(:topical_event_featuring_image_data)
    topical_event.feature(edition_id: edition_2.id, alt_text: "A thing", image: image_2, ordering: 0)
