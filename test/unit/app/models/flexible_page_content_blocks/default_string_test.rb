...
class FlexiblePageContentBlocks::DefaultStringRenderingTest < ActionView::TestCase
  ...
  test "the form label is equal to the attribute title" do
    render html: @block.render(@schema["properties"]["test_attribute"], @page.flexible_page_content["test_attribute"], Path.new.push("test_attribute"))
  ...
  test "it add a required message to the label when the attribute is required" do
    render html: @block.render(@schema["properties"]["test_attribute"], @page.flexible_page_content["test_attribute"], Path.new.push("test_attribute"), required: true)
