...
class Edition::WorldwideOrganisationsTest < ActiveSupport::TestCase
  ...
  test "#destroy removes association with organisations" do
    ...
    edition = create(:draft_case_study, legacy_worldwide_organisations: [organisation])
    edition.destroy!
    assert_not edition.edition_worldwide_organisations.any?
    assert LegacyWorldwideOrganisation.exists?(organisation.id)
  end

  test "new editions carry over worldwide organisations" do
    organisation = create(:worldwide_organisation)
    edition = create(:published_case_study, legacy_worldwide_organisations: [organisation])
    new_edition = edition.create_draft(create(:writer))
    new_edition.change_note = "change-note"
    force_publish(new_edition)

    assert_equal [organisation], new_edition.legacy_worldwide_organisations
  end
end
