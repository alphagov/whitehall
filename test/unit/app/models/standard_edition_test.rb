...
class StandardEditionTest < ActiveSupport::TestCase
  ...
  test "it allows lead image behaviour if the schema permits it" do
    test_type_with_lead_image =
      build_configurable_document_type(
        "test_type_with_lead_image", {
          "settings" => {
            "lead_image_enabled" => true,
          },
        }
      )
    test_type_without_lead_image =
      build_configurable_document_type(
        "test_type_without_lead_image", {
          "settings" => {
            "lead_image_enabled" => false,
          },
        }
      )

    ConfigurableDocumentType.setup_test_types(test_type_with_lead_image.merge(test_type_without_lead_image))
    page_with_lead_image = StandardEdition.new(configurable_document_type: "test_type_with_lead_image")
    page_without_lead_image = StandardEdition.new(configurable_document_type: "test_type_without_lead_image")
    assert page_with_lead_image.can_have_lead_image?
    assert_not page_without_lead_image.can_have_lead_image?
  end
