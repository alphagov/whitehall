...
class LandingPageBodyTest < ActiveSupport::TestCase
  ...
  test "is invalid with empty YAML" do
    subject = PlanForChangeLandingPage::Body.new("", EMPTY_IMAGES)
  ...
  test "is invalid with empty blocks" do
    subject = PlanForChangeLandingPage::Body.new(<<~YAML, EMPTY_IMAGES)
      blocks: []
    YAML
    assert subject.invalid?
    assert_equal ["Blocks cannot be blank"], subject.errors.to_a
  ...
  test "extends a document which does exist" do
    edition = create(:edition, type: "PlanForChangeLandingPage", body: "navigation_groups: []", slug_override: "/some-slug")
    subject = PlanForChangeLandingPage::Body.new(<<~YAML, EMPTY_IMAGES)
