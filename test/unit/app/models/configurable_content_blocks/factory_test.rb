...
class ConfigurableContentBlocks::FactoryTest < ActiveSupport::TestCase
  ...
  test "it builds a govspeak string block with empty attachments if the edition does not support inline attachments" do
    ...
    ConfigurableDocumentType.setup_test_types(test_type_without_inline_attachments)
    page = StandardEdition.new(configurable_document_type: "test_type_without_inline_attachments")
    page.attachments = [build(:file_attachment)]
    factory = ConfigurableContentBlocks::Factory.new(page)
    block = mock("ConfigurableContentBlocks::Govspeak")
