...
class ValidateConfigurableDocumentSchemasTest < ActiveSupport::TestCase
  ...
  describe "`SchemaValidator` should validate" do
    ...
    context "within `schema`" do
      it "that all defined `attributes` are used within `forms`" do
      ...
      it "all `validations` only reference fields that are defined in `attributes`" do
    ...
    context "within `forms`" do
      it "that all defined `fields` are also defined in `schema`" do
