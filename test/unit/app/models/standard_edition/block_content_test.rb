...
class BlockContentTest < ActiveSupport::TestCase
  ...
  test "stores whatever value is provided for object attributes without casting" do
    ...
    page.attributes = { "test_object_attribute" => { "selected" => %w[england] } }
    assert_equal({ "selected" => %w[england] }, page.test_object_attribute)
  end
  ...
  test "maps 'nation_applicability' validation to NationApplicabilityValidator" do
    ...
    page.attributes = { "test_object_attribute" => { "selected" => %w[not_a_nation] } }
    assert_not page.valid?
    assert_not page.errors.where("test_object_attribute", :invalid).empty?
  end
end
