...
class AttachmentDataTest < ActiveSupport::TestCase
  ...
  test "#deleted? returns false for AttachmentData with multiple attachments, one belonging to live attachable" do
  ...
  test "#deleted? returns true for AttachmentData with multiple attachments, if they're all on live attachables" do
    attachable = create(:published_news_article, :with_file_attachment)
    new_attachable = attachable.reload.create_draft(create(:gds_editor))
    attachment = new_attachable.attachments.first
    attachment.destroy!
