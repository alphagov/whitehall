...
class ConfigurableAssociations::FactoryTest < ActiveSupport::TestCase
  ...
  test "configurable_associations raises an error if the association does not exist - with forms" do
    ...
    configurable_document_type = build_configurable_document_type_with_forms("test_type", { "associations" => association_config })
    ConfigurableDocumentType.setup_test_types(configurable_document_type)
  ...
  test "configurable_associations builds all of the configured associations for an edition - with forms" do
    association_config = [
      {
        "key" => "ministerial_role_appointments",
      },
