...
class BulkUploadTest < ActiveSupport::TestCase
  ...
  test "#save_attachments will not create a new file for a kept attachment if there are validation errors with other attachments" do
    edition = create(:news_article, :with_file_attachment)
    existing = edition.attachments.first
    params = attachments_params(
      [{ id: existing.id, title: "Title 1" }, { file: upload_fixture(existing.filename), keep_or_replace: "keep", new_filename: "new_filename.pdf" }],
      [{ title: nil }, { file: upload_fixture("simple.pdf") }],
    )
    ...
    assert_equal bulk_upload.attachments.first.attachment_data.filename, "greenpaper.pdf"
  end
