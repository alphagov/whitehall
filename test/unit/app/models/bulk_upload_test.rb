...
class BulkUploadTest < ActiveSupport::TestCase
  ...
  test "loads attachments from the edition if filenames match" do
    ...
    paths = ["whitepaper.pdf", existing.filename].map { |name| upload_fixture(name) }
    bulk_upload = BulkUpload.new(edition, files: paths)
  ...
  test "builds new AttachmentData when existing file re-attached from attachments_attributes" do
    ...
    bulk_upload = BulkUpload.new(edition, attachments_params: params)
  ...
  test "#save_attachments saves new attachments to the end of the edition's existing attachments" do
    ...
    params = attachments_params(
      [{ title: "Title 1" }, { file: upload_fixture("whitepaper.pdf"), attachable: edition }],
      [{ title: "Title 2" }, { file: upload_fixture("simple.pdf"), attachable: edition }],
    )
  ...
  test "#save_attachments updates existing attachments" do
    ...
    params = attachments_params(
      [{ id: existing.id, title: new_title }, { file: upload_fixture(existing.filename), attachable: edition }],
    )
  ...
  test "#save_attachments does not save any attachments if one is invalid" do
    edition = create(:news_article)
    bulk_upload = BulkUpload.new(edition, attachments_params: invalid_new_attachments_params)
