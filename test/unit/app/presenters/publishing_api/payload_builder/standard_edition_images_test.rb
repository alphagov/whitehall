    ...
    class StandardEditionImagesTest < ActiveSupport::TestCase
      ...
      context "usage: lead" do
        ...
        test "allows additional non-embeddable usages alongside lead" do
          ...
          assert result.is_a?(Array)
          assert_equal 2, result.count
          assert_equal lead_image.url(:s960), result.select { |img| img[:type] == "lead" }.first[:sources][:s960]
          assert_equal non_embeddable_image.url, result.select { |img| img[:type] == "non_embeddable_usage" }.first[:sources][:s960]
        end
      end
    end
  end
