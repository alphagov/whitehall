...
class RoleTest < ActiveSupport::TestCase
  ...
  test 'should be invalid if status is empty' do
    new_role = build(:role, status: '')
    refute new_role.valid?
  end
  test 'should be invalid if status is not an expected value' do
    new_role = build(:role, status: 'notvalid')
    refute new_role.valid?
  end
