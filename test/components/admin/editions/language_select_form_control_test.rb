...
class Admin::Editions::LanguageSelectFormControlTest < ViewComponent::TestCase
  ...
  test "shows a hint that the shown language cannot be changed after the document has been translated" do
    ConfigurableDocumentType.setup_test_types(build_configurable_document_type("test_type", { "settings" => { "translations_enabled" => true } }))
    edition = create(:draft_standard_edition)
    edition.translations.create!(locale: :fr)
    render_inline(Admin::Editions::LanguageSelectFormControl.new(edition))
    assert_selector "#edition_primary_locale_hint", text: "The language cannot be changed as this document has been translated"
  end

  test "displays the selected locale as readonly once the edition has a translation" do
    ConfigurableDocumentType.setup_test_types(build_configurable_document_type("test_type", { "settings" => { "translations_enabled" => true } }))
    edition = create(:draft_standard_edition)
    edition.translations.create!(locale: :fr)
    render_inline(Admin::Editions::LanguageSelectFormControl.new(edition))
    assert_selector "input[value=\"English\"][readonly=\"readonly\"]"
  end
end

class Admin::Editions::LanguageSelectFormControlTestWithForms < ViewComponent::TestCase
  test "it does not render if translations are disabled for the edition" do
    ConfigurableDocumentType.setup_test_types(build_configurable_document_type_with_forms("test_type", { "settings" => { "translations_enabled" => false } }))
    edition = build(:draft_standard_edition)
    assert_not Admin::Editions::LanguageSelectFormControl.new(edition).render?
  end

  test "renders a select input with all available locales as options" do
    ConfigurableDocumentType.setup_test_types(build_configurable_document_type_with_forms("test_type", { "settings" => { "translations_enabled" => true } }))
    edition = build(:draft_standard_edition)
    render_inline(Admin::Editions::LanguageSelectFormControl.new(edition))
