...
class ReleaseAnnouncementsControllerTest < ActionController::TestCase
  test "#index should assign filtered release announcments" do
    Frontend::ReleaseAnnouncementProvider.stubs(:find_by).with({ keywords: "womble" }).returns(:some_filtered_release_announcements)
    get :index, release_announcements_filter: { keywords: "womble" }
  ...
  test "#index assign a ReleaseAnnouncementsFilter, populated with get params" do
    get :index, release_announcements_filter: { keywords: "wombats",
