...
class Admin::SupportingDocumentsControllerTest < ActionController::TestCase
  ...
  test "shows the title and a link back to the parent" do
    document = create(:draft_policy)
    supporting_page = create(:supporting_document, document: document)
  ...
  test "shows the body using govspeak markup" do
    supporting_page = create(:supporting_document, body: "body-in-govspeak")
  ...
  test "edit form has title and body inputs" do
    ...
    assert_select "form[action='#{admin_supporting_document_path(supporting_page)}']" do
      assert_select "input[name='supporting_document[title]'][type='text'][value='#{supporting_page.title}']"
      assert_select "textarea[name='supporting_document[body]']", text: supporting_page.body
      assert_select "input[type='submit']"
    end
  end
  ...
  test "edit form include lock version to prevent conflicting changes overwriting each other" do
    ...
    assert_select "form[action='#{admin_supporting_document_path(supporting_page)}']" do
      assert_select "input[name='supporting_document[lock_version]'][type='hidden'][value='#{supporting_page.lock_version}']"
    end
  end

  test "update modifies supporting document" do
    supporting_page = create(:supporting_document)

    attributes = { title: "new-title", body: "new-body" }
    put :update, id: supporting_page, supporting_document: attributes
  ...
  test "updating a stale supporting document should render edit page with conflicting supporting document" do
    ...
    attributes = { title: "new-title", body: "new-body" }
    put :update, id: supporting_page, supporting_document: attributes.merge(lock_version: lock_version)
  ...
  test "should be able to destroy a destroyable supporting document" do
    document = create(:draft_policy)
    supporting_page = create(:supporting_document, document: document, title: "Blah blah")

    delete :destroy, id: supporting_page.id

    assert_redirected_to admin_policy_path(document)
    refute SupportingDocument.find_by_id(supporting_page.id)
  ...
  test "destroying an indestructible role" do
    ...
    assert_redirected_to admin_policy_path(document)
    assert SupportingDocument.find_by_id(supporting_page.id)
