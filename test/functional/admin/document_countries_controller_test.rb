...
class Admin::DocumentCountriesControllerTest < ActionController::TestCase
  ...
  test 'should allow featuring of the document country' do
    document_country = create(:edition_country, featured: false)
    login_as :departmental_editor
    post :update, id: document_country, edition_country: {featured: true}
    assert document_country.reload.featured?
  end

  test 'should allow unfeaturing of the document country' do
    document_country = create(:edition_country, featured: true)
    login_as :departmental_editor
    post :update, id: document_country, edition_country: {featured: false}
    refute document_country.reload.featured?
  end

  test "should redirect back to the country's admin edit page" do
    country = create(:country)
    document_country = create(:edition_country, country: country)
    login_as :departmental_editor
    post :update, id: document_country, edition_country: {}
