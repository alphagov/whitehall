...
class Admin::EditionImagesControllerTest < ActionController::TestCase
  ...
  view_test "POST :create shows a validation error if one 'single' usage image is too small, using the 'new' template" do
    ...
    assert_select ".govuk-error-summary li", "Image \"50x33_gif.gif\" is too small. Select an image that is at least #{topical_event_header_kind.valid_width} pixels wide and at least #{topical_event_header_kind.valid_height} pixels tall."
  end
  ...
  view_test "POST :create shows a validation error if one 'multiple' usage embeddable image is too small, using the 'index' template" do
    ...
    assert_template "admin/edition_images/index"
    assert_select ".govuk-error-summary li", "Image \"50x33_gif.gif\" is too small. Select an image that is at least #{default_kind.valid_width} pixels wide and at least #{default_kind.valid_height} pixels tall."
  end
  ...
  view_test "POST :create shows a validation error if one 'multiple' usage embeddable image has a duplicated filename, using the 'index' template" do
    ...
    assert_template "admin/edition_images/index"
    assert_select ".govuk-error-summary li", "Image \"960x640_gif.gif\" is not unique. All your file names must be different. Do not use special characters to create another version of the same file name."
  end
  ...
  test "DELETE :destroy when a lead image is present it deletes the edition_lead_image and sets a new lead image" do
    ...
    image1 = build(:image)
    image2 = build(:image)
    edition = create(:draft_standard_edition, images: [image1, image2])
