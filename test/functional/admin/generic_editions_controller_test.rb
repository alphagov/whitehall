...
class Admin::GenericEditionsControllerTest < ActionController::TestCase
  ...
  view_test "GET :edit shows the similar slug warning as an error which links to the input when user has 'Preview design system' permission" do
    ...
    assert_select ".govuk-error-summary a", text: "Title has been used before on GOV.UK, although the page may no longer exist. Please use another title. If not, the URL for this #{edition_with_same_title.format_name} will be #{edition_with_same_title.base_path}", href: "#edition_title"
  end
