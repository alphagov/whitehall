...
class Admin::EditionTranslationsControllerTest < ActionController::TestCase
  ...
  test "#confirm_destroy returns a redirect with a flash message for a translation associated with a published standard edition" do
    edition = build(:published_standard_edition)
    setup_configurable_document_type("test_type", { "title" => "Translation Test Type" })
    with_locale(:fr) { edition.update!(title: "french-title", summary: "french-summary", body: "french-body") }

    get :confirm_destroy, params: { edition_id: edition, id: "fr" }

    assert_redirected_to @controller.admin_edition_path(edition)
    assert_equal "You cannot modify a #{edition.state} Translation Test Type", flash[:alert]
  end
  ...
  test "#destroy returns a redirect with a flash message for a translation associated with a published standard edition" do
    edition = build(:published_standard_edition)
    setup_configurable_document_type("test_type", { "title" => "Translation Test Type" })
    with_locale(:fr) { edition.update!(title: "french-title", summary: "french-summary", body: "french-body") }
