...
class Admin::StandardEditionsControllerTest < ActionController::TestCase
  ...
  context "Tests using forms schema for configurable content blocks" do
    ...
    view_test "GET choose_type scopes the list of types to types that the user has permission to use" do
      configurable_document_type_user_org = build_configurable_document_type_with_forms("test_type", { "title" => "Test Type One", "settings" => { "organisations" => [@current_user.organisation.content_id] } })
      configurable_document_type_other_org = build_configurable_document_type_with_forms("other_type", { "title" => "Test Type Two", "settings" => { "organisations" => [SecureRandom.uuid] } })
    ...
    view_test "GET choose_type displays all types if the group is 'all'" do
      type_one = build_configurable_document_type_with_forms("type_one", { "title" => "Type One", "settings" => { "configurable_document_group" => "one" } })
      type_two = build_configurable_document_type_with_forms("type_two", { "title" => "Type Two", "settings" => { "configurable_document_group" => "two" } })
      ConfigurableDocumentType.setup_test_types({}.merge(type_one, type_two))
      get :choose_type, params: { group: "all" }
      assert_response :ok
      assert_dom "h1", "New standard document"
    ...
    view_test "GET choose_type displays types from group" do
      type_one = build_configurable_document_type_with_forms("type_one", { "title" => "Type One", "settings" => { "configurable_document_group" => "group_one" } })
      type_two = build_configurable_document_type_with_forms("type_two", { "title" => "Type Two", "settings" => { "configurable_document_group" => "group_two" } })
      ConfigurableDocumentType.setup_test_types({}.merge(type_one, type_two))
      get :choose_type, params: { group: "group_two" }
      assert_response :ok
      assert_dom "h1", "New group two"
    ...
    view_test "GET change_type shows only permitted sibling types in the same group as the current edition's type" do
      current_type = build_configurable_document_type_with_forms("current_type", { "title" => "Current Type", "settings" => { "configurable_document_group" => "group_type" } })
      permitted_sibling_type = build_configurable_document_type_with_forms("permitted_sibling_type", { "title" => "Permitted Sibling Type", "settings" => { "configurable_document_group" => "group_type", "organisations" => [@current_user.organisation.content_id] } })
      non_permitted_sibling_type = build_configurable_document_type_with_forms("non_permitted_sibling_type", { "title" => "Non-permitted Sibling Type", "settings" => { "configurable_document_group" => "group_type", "organisations" => [SecureRandom.uuid] } })
      permitted_non_sibling_type = build_configurable_document_type_with_forms("permitted_non_sibling_type", { "title" => "Permitted Non-sibling Type", "settings" => { "organisations" => [@current_user.organisation.content_id] } })
      ConfigurableDocumentType.setup_test_types(
        current_type.merge(permitted_sibling_type)
          .merge(non_permitted_sibling_type)
          .merge(permitted_non_sibling_type),
    ...
    view_test "GET change_type_preview shows the document type change preview" do
      old_type = build_configurable_document_type_with_forms("old_type", { "title" => "Old Type" })
      new_type = build_configurable_document_type_with_forms("new_type", { "title" => "New Type", "settings" => { "configurable_document_group" => "group_type" } })
      ConfigurableDocumentType.setup_test_types(old_type.merge(new_type))
      edition = create(:standard_edition, configurable_document_type: "old_type")
      get :change_type_preview, params: { id: edition.id, configurable_document_type: "new_type" }
      assert_response :ok
    ...
    view_test "PATCH apply_change_type succeeds if user has permission and params are valid" do
      original_document_type = build_configurable_document_type_with_forms("original_document_type")
      new_document_type = build_configurable_document_type_with_forms("new_document_type")
      ConfigurableDocumentType.setup_test_types(original_document_type.merge(new_document_type))
      edition = create(:draft_standard_edition, configurable_document_type: "original_document_type")
      StandardEdition.any_instance.stubs(:update_configurable_document_type).returns(true)
    ...
    view_test "GET edit renders previously published form controls if backdating is enabled" do
      configurable_document_type = build_configurable_document_type_with_forms("test_type", { "settings" => { "backdating_enabled" => true } })
      ConfigurableDocumentType.setup_test_types(configurable_document_type)
    ...
    view_test "GET edit renders the history mode form controls when history mode is enabled" do
      configurable_document_type = build_configurable_document_type_with_forms("test_type", { "settings" => { "history_mode_enabled" => true } })
      ConfigurableDocumentType.setup_test_types(configurable_document_type)
    ...
    view_test "GET edit renders Attachments tab and alternative format provider select when file attachments are enabled" do
      configurable_document_type = build_configurable_document_type_with_forms("test_type", { "settings" => { "file_attachments_enabled" => true } })
      ConfigurableDocumentType.setup_test_types(configurable_document_type)
    ...
    view_test "GET edit renders the form controls for the configured associations" do
      configurable_document_type = build_configurable_document_type_with_forms("test_type", { "associations" => [
        {
          "key" => "ministerial_role_appointments",
        },
        {
