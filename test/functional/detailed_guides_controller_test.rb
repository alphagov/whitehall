...
That's all
  ...
  test "search lists each result returned from the inside government client" do
    ...
    Whitehall.search_client.stubs(:search).with('query', 'detailed_guidance').returns([{"title" => "title", "link" => "/detailed/guide-slug", "highlight" => "", "presentation_format" => "detailed_guidance"}])
    get :search, q: 'query'
  ...
  test "autocomplete returns the response from autocomplete as a string" do
    ...
    Whitehall.search_client.stubs(:autocomplete).with("search-term", "detailed_guidance").returns(raw_rummager_response)
