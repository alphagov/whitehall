...
- The `errors` (default `[]`) attribute. The valdiation errors for the edition. Use the `errors_for` helper function and pass it the errors and the attribute path to pass the attribute errors to the form control component.
...
Rails validations can be applied to properties by adding a `validations` key to the property's object schema. The value for the `validations` key should be an object. The keys for the object must match a value for this the case statement in [the block content type class'](../app/models/standard_editions/block_content_type.rb) `valid_instance_of_document_type_attributes` method. The value for each key is an object which will be passed to the validator constructor. The `attributes` must be included and must be an array of the names of the attributes to be validated, and other options may be passed depending on what arguments the validator initialize method accepts. 
