...
| allowed_child_document_types | N                                                                                               | An array of objects describing which document types (by their `key`) are allowed to be created as children of this document type. E.g. `[{ "document_type": "mini_site_child" }]`. Defaults to no allowed child types.                                        |
