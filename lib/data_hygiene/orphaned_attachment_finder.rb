  ...
  class OrphanedAttachmentFinder
    ...
    def summarize_by_type
      "Documents with all/some attachments not referenced by inline tags:\n\n" +
        format("%-30s %-11s %-11s", "", "Published", "Other") + "\n" +
        format("%-30s %5s %5s %5s %5s", "", "All--", "Some-", "All--", "Some-") + "\n" +
        editions_with_orphaned_attachments
          ...
          .map { |class_name, batch|
            ...
            format("%-30s %5s %5s %5s %5s", class_name, all_missing_published.size, some_missing_published.size, all_missing_other.size, some_missing_other.size)
          }
          .join("\n")
    end
