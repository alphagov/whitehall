  ...
  AttachmentData.find_each.map do |attachment_data|
    ...
    attachment_data.assets.map do |asset|
      ...
      if attachment_data.replaced_by_id
        replaced = !!asset_manager_response["replacement_id"]
        ...
        puts "AttachmentData #{attachment_data.id} deleted in WH, asset #{asset.asset_manager_id} not deleted in AM, redirect: #{!!asset_manager_response["redirect_url"]}"
