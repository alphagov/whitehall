...
namespace :export do
  ...
  task :document_list => :environment do
    ...
    document_identities = DocumentIdentity.includes(:latest_edition).where('document_identities.created_at >= :created_since', created_since: created_since)
    documents = document_identities.map(&:latest_edition).compact

    CSV do |csv|
      csv << ["Title", "Admin URL", "State", "Type", "Authors"]
      documents.each do |document|
        author_names = document.document_authors.map(&:user).uniq.map(&:name)
