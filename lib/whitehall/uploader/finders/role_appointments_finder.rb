class Whitehall::Uploader::Finders::RoleAppointmentsFinder
  def self.find(date, *slugs, logger, line_number)
    ...
    people = slugs.map do |slug|
      ...
      logger.error "Unable to find Person with slug '#{slug}'" unless person
      person
    end.compact

    people.map do |person|
      appointments = person.role_appointments_at(date)
      logger.error "Unable to find an appointment for '#{person.slug}' at '#{date}'" if appointments.empty?
      appointments
    end.flatten
  end
end
