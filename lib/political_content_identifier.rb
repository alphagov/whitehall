...
private
  ...
  def potentially_political_standard_edition?
    edition.is_a?(StandardEdition) && %w[news_story press_release].include?(edition.configurable_document_type)
  end
  ...
  def always_political_format?
    edition.is_a?(StandardEdition) && edition.configurable_document_type == "world_news_story"
  end
