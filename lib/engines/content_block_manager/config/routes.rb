  ...
  namespace :content_block_manager, path: "/" do
    ...
    namespace :content_block, path: "content-block" do
      ...
      resources :documents, only: %i[index show new], path_names: { new: "(:block_type)/new" }, path: "" do
        ...
        resources :embedded_objects, only: %i[new create], path_names: { new: "(:object_type)/new" }, controller: "documents/embedded_objects" do
          collection do
            post :new_embedded_objects_options_redirect
          end
        end
