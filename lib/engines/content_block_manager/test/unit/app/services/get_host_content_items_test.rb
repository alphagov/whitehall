...
class ContentBlockManager::GetHostContentItemsTest < ActiveSupport::TestCase
  ...
  describe "#items" do
    context "when BIGQUERY_PROJECT_ID is not present" do
      around do |test|
        ClimateControl.modify BIGQUERY_PROJECT_ID: "" do
          test.call
        end
      end

      setup do
        ContentBlockManager::PageViewsService::Local.stubs(:new).with(paths: ["/foo"]).returns(stub(call: []))
      end
      ...
      context "when PageViewsService has data for a page" do
        ...
        setup do
          ContentBlockManager::PageViewsService::Local.stubs(:new).with(paths: ["/foo"]).returns(stub(call: pageviews))
        end
    ...
    context "when BIGQUERY_PROJECT_ID is present" do
      around do |test|
        ClimateControl.modify BIGQUERY_PROJECT_ID: "something" do
          test.call
        end
      end

      setup do
        ContentBlockManager::PageViewsService::Google.stubs(:new).with(paths: ["/foo"]).returns(stub(call: []))
      end

      it "returns zero page views by default" do
        result = described_class.by_embedded_document(content_block_document: document)
      ...
      context "when PageViewsService has data for a page" do
        ...
        setup do
          ContentBlockManager::PageViewsService::Google.stubs(:new).with(paths: ["/foo"]).returns(stub(call: pageviews))
        end
