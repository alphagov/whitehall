...
class ContentBlockManager::ConfirmationCopyPresenterTest < ActiveSupport::TestCase
  ...
  context "when the content block is scheduled" do
    let(:content_block_edition) { build(:content_block_edition, :email_address, scheduled_publication: Time.zone.now) }

    describe "#for_panel" do
      it "should return the scheduled text" do
        assert_equal "#{content_block_edition.block_type.humanize} scheduled to publish on #{I18n.l(content_block_edition.scheduled_publication, format: :long_ordinal)}", presenter.for_panel
      end
    end
  ...
  context "when there is more than one edition for the underlying document" do
    ...
    describe "#for_panel" do
      it "should return the published text" do
        assert_equal "#{content_block_edition.block_type.humanize} published", presenter.for_panel
      end
    end
  ...
  context "when there is only one edition for the underlying document" do
    describe "#for_panel" do
      it "should return the created text" do
        assert_equal "#{content_block_edition.block_type.humanize} created", presenter.for_panel
      end
    end
