module ContentObjectStore
  class SchemaService
    def self.schema_for_block_type(block_type)
      schema_name = "#{Schema::SCHEMA_PREFIX}_#{block_type}"
    ...
    def self.valid_schemas
      @valid_schemas ||= Services.publishing_api.get_schemas.select { |k, _v|
        k.start_with?(Schema::SCHEMA_PREFIX)
      }.map do |k, v|
    ...
    def self.initialize_schema(id, full_schema)
      schema = full_schema.dig("definitions", "details")
      ContentObjectStore::Schema.new(id, schema)
    end
  end
end
