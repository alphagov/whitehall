...
class ContentObjectStore::ContentBlock::WorkflowTest < ActionDispatch::IntegrationTest
  ...
  describe "when creating a new content block" do
    describe "when reviewing the changes" do
      let(:step) { ContentObjectStore::ContentBlock::Editions::WorkflowController::NEW_BLOCK_STEPS[:review] }

      describe "#show" do
        it "shows the new edition for review" do
          get content_object_store.content_object_store_content_block_workflow_path(id: edition.id, step:)

          assert_template "content_object_store/content_block/editions/workflow/review"
      ...
      describe "#update" do
        it "posts the new edition to the Publishing API and marks edition as published" do
          assert_edition_is_published do
            put content_object_store.content_object_store_content_block_workflow_path(id: edition.id, step:)
          end
        end
      end
    end
  ...
  describe "when updating an existing content block" do
    describe "when reviewing the links" do
      let(:step) { ContentObjectStore::ContentBlock::Editions::WorkflowController::UPDATE_BLOCK_STEPS[:review_links] }

      describe "#show" do
        it "shows host content items" do
          host_content_items = build_list(:host_content_item, 2)
          ContentObjectStore::GetHostContentItems.expects(:by_embedded_document)
                                                 .with(content_block_document: document)
                                                 .returns(host_content_items)

          get content_object_store.content_object_store_content_block_workflow_path(id: edition.id, step:)

          assert_template "content_object_store/content_block/editions/workflow/review_links"
      ...
      describe "#update" do
        it "redirects to the next step" do
          ...
          assert_redirected_to content_object_store_content_block_workflow_path(id: edition.id, step: :schedule_publishing)
        end
      end
    end

    describe "when scheduling or publishing" do
      let(:step) { ContentObjectStore::ContentBlock::Editions::WorkflowController::UPDATE_BLOCK_STEPS[:schedule_publishing] }

      describe "#show" do
        it "shows the form" do
          get content_object_store.content_object_store_content_block_workflow_path(id: edition.id, step:)

          assert_template "content_object_store/content_block/editions/workflow/schedule_publishing"
      ...
      describe "#update" do
        describe "when choosing to publish immediately" do
          it "posts the new edition to the Publishing API and marks edition as published" do
            assert_edition_is_published do
              put content_object_store.content_object_store_content_block_workflow_path(id: edition.id, step:), params: { schedule_publishing: "now" }
            end
          end
        end
        ...
        describe "when scheduling publication" do
          it "schedules the edition and redirects" do
            ...
            ContentObjectStore::ScheduleEditionService.any_instance.expects(:call).with(edition, ActionController::Parameters.new(scheduled_at).permit!)

            put content_object_store.content_object_store_content_block_workflow_path(id: edition.id, step:), params: {
              schedule_publishing: "schedule",
              scheduled_at:,
            }

            assert_redirected_to content_object_store_content_block_document_path(document)
