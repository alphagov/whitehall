describe('GOVUK.analyticsGa4.analyticsModules.Ga4FinderSetup', function () {
  ...
  clearAllLink.setAttribute('data-ga4-filter-type', 'filter-type')
  clearAllLink.innerHTML = `
    <a href='/' data-ga4-change-category="clear-all-filters">
      Link Text
    </a>
  `
  ...
  describe('updates dataset correctly', () => {
    it('for a clear all fields link', () => {
      form.innerHTML = clearAllLink.outerHTML
      ...
      const expectedDataset = {
        ga4ChangeCategory: 'clear-all-filters',
        ga4Link: JSON.stringify({
          action: 'remove',
    ...
    it('for a checkbox', () => {
      form.innerHTML = checkboxes.outerHTML
      ...
      expect({ ...input.dataset }).toEqual(expectedDataset)
    })

    it('for a radio', () => {
      form.innerHTML = radios.outerHTML
    ...
    describe('for a select', () => {
      ...
      it('that are single choice', () => {
        ...
        expect({ ...selectInput.dataset }).toEqual(expectedDataset)
      })
