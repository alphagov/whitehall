...
private
  ...
  def validate_attribute_contains_no_footnotes(attribute_name)
    if @record.public_send(attribute_name) =~ FOOTNOTE_TAG_MATCHER
      @record.errors.add(attribute_name, :no_footnotes_allowed, message: "cannot include footnotes on this type of document (#{attribute_name.to_s.humanize} includes '#{Regexp.last_match(1)}')")
    end
  end
end
