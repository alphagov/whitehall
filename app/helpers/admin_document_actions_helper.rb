  ...
  def force_publish_document_button(document)
    capture do
      form_for [:admin, document], {as: :document, url: admin_document_publishing_path(document, force: true), method: :post} do |form|
        ...
        submit_options = {title: "Publish #{document.title}"}
        if document.has_supporting_documents?
          submit_options[:confirm] = "Are you sure you want to force publish this document? Have you checked the #{document.supporting_documents.count} supporting documents?"
        else
