class FeaturedImageData < ApplicationRecord
  ...
  def publishing_api_details
    {
      sources: image_kind_config.versions.reduce({}) { |sources, version| url(version.name) ? sources.merge({ version.name.to_sym => url(version.name) }) : sources.merge({ version.name.to_sym => url }) },
      content_type:,
    }
  end
