...
class Edition < ApplicationRecord
  ...
  UNMODIFIABLE_STATES = %w[scheduled published superseded deleted removed].freeze
  FROZEN_STATES = %w[superseded deleted removed].freeze
  PRE_PUBLICATION_STATES = %w[draft submitted rejected scheduled].freeze
  POST_PUBLICATION_STATES = %w[published superseded withdrawn removed].freeze
  ...
  class UnmodifiableValidator < ActiveModel::Validator
    ...
    def being_unpublished?(previous_state, current_state)
      previous_state == "published" && %w[removed withdrawn].include?(current_state)
    end
  end
