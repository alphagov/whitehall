class SearchIndexAddJob < JobBase
  ...
  def perform(class_name, id)
    ...
    if searchable_instance.nil?
      logger.warn("SearchIndexAddJob: Could not find #{class_name} with id #{id} (#{Time.zone.now.utc}).")
    elsif !searchable_instance.can_index_in_search?
      logger.warn("SearchIndexAddJob: Was asked to index #{class_name} with id #{id}, but it was unindexable (#{Time.zone.now.utc}).")
    else
