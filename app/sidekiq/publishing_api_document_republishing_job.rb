...
# This job synchronises the state of the editions for a document with the
...
class PublishingApiDocumentRepublishingJob < JobBase
...
private
  ...
  def republish_latest_unpublished_edition
    ...
    PublishingApiUnpublishingJob.new.perform(edition.unpublishing.id, true)
  ...
  def republish_withdrawn_edition
    ...
    PublishingApiUnpublishingJob.new.perform(edition.unpublishing.id, edition.draft?)
    handle_attachments_for(edition)
  end
