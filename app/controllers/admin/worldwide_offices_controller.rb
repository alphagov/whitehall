class Admin::WorldwideOfficesController < Admin::BaseController
  ...
  def update
    ...
    if @worldwide_office.update(worldwide_office_params)
      ...
      redirect_to admin_legacy_worldwide_organisation_worldwide_offices_path(@worldwide_organisation), notice: "#{@worldwide_office.title} has been edited"
    else
  ...
  def create
    ...
    if @worldwide_office.save
      handle_show_on_home_page_param
      redirect_to admin_legacy_worldwide_organisation_worldwide_offices_path(@worldwide_organisation), notice: "#{@worldwide_office.title} has been added"
    else
      @worldwide_office.contact.contact_numbers.build if @worldwide_office.contact.contact_numbers.blank?
      render :new
    end
