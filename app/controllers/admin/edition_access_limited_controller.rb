...
private
  ...
  def assign_access_limiting_organisations
    ...
    return unless submitted_org_ids.any?

    @edition.access_limiting_organisations = Organisation.where(id: submitted_org_ids)
  end

  def access_limiting_organisations_invalid?
    return false unless Flipflop.access_limiting_organisations_ui?
    return false unless submitted_org_ids.empty?
    return false unless @edition.access_limiting_organisations?
  ...
  def apply_access_limiting_organisations
    return unless Flipflop.access_limiting_organisations_ui?
    return unless submitted_org_ids.empty?

    @edition.edition_access_limiting_organisations.each(&:mark_for_destruction)
  end

  def submitted_org_ids
