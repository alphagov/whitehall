# Statistics Announcements are supposed to automatically unpublish when their associated publication goes live, but some have been left in a published state. This migration unpublishes those announcements and republishes them as redirects to the statistics announcements index page.
...
statistics_announcements_with_live_publications.each { |sa| Whitehall::PublishingApi.publish_redirect_async(sa.content_id, sa.publication.public_url) }
...
deleted_statistics_announcements.each do |sa|
  sa.update_columns(
    redirect_url: "https://www.gov.uk/government/statistics/announcements",
