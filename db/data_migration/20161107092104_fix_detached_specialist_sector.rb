...
puts format("Found %<count>d Specialist Sectors with a missing `topic_content_id`", count: specialist_sectors.count)
...
specialist_sectors.each do |specialist_sector|
  ...
  if (topic = linkable_topic_items.find { |item| item["base_path"] == "/topic/#{specialist_sector['tag']}" })
    ...
    puts format("  %<status>s `content_id` %<content_id>s", content_id: specialist_sector.topic_content_id = topic["content_id"], status: specialist_sector.save ? "Assigned" : "Failed to assign")
  else
    puts "  No topic found"
  end
end
