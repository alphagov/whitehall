...
DataHygiene::DuplicateAttachmentFinder.new.non_editions.each do |model|
  ...
  logger.info("\tAttachments before:\t#{model.attachments.map(&:filename).to_sentence}")
  DataHygiene::DupFilenameAttachmentFixer.new(model).run!
  logger.info("\tAttachments after:\t#{model.reload.attachments.map(&:filename).to_sentence}")
end
