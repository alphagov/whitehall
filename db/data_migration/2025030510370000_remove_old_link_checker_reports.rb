...
overloaded_edition_ids = LinkCheckerApiReport.group(:edition_id)
  .having("COUNT(*) > 1")
  .pluck(:edition_id)
  .compact
  .uniq
edition_ids_count = overloaded_edition_ids.count

overloaded_edition_ids.each_with_index do |edition_id, index|
  ...
  reports_to_delete_count = reports_to_delete.count
  ...
  puts "#{index + 1}/#{edition_ids_count} - Edition ID #{edition_id}: latest link_checker_api_report ID is #{latest_report.id} (updated at: #{latest_report.updated_at}). Deleted #{reports_to_delete_count} older reports."
end
