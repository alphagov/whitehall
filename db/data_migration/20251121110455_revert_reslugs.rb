...
documents = Document.where("updated_at > ?", date_of_merge_of_reslug_changes).where("updated_at < ?", date_of_merge_of_revert_changes)
...
documents.each do |document|
  ...
  new_slug = normalize_friendly_id(published_edition.title) # TODO: investigate how we generate the slug in current code

  if document.slug == new_slug
    puts "SKIP: Old slug #{document.slug} and new slug from title #{new_slug} are the same"
    next
  end

  begin
    DataHygiene::DocumentReslugger.new(document, published_edition, user, new_slug).run!
    puts "SUCCESS: Reslugged document #{document.content_id} from old slug #{document.slug} to new slug #{new_slug}"
  rescue StandardError => e
    puts "ERROR: Could not reslug document #{document.content_id} from old slug #{document.slug} to new slug #{new_slug}: #{e}"
  end
end
