govspeak_embed_image_count = Image.joins(:edition).where.not(editions: { type: "LandingPage" }).where(usage: nil).update_all(usage: "govspeak_embed")

puts "Set usage for #{govspeak_embed_image_count} govspeak embedding images"

landing_page_images = Image.joins(:edition).where(editions: { type: "LandingPage" }).where(usage: nil)
