  ...
  def up
    ...
    ActiveRecord::Base.transaction do
      WorldwideOrganisation.find_each do |worldwide_organisation|
        ## Output some information which will help with debugging
        ...
        ## The 'About us' page will get merged into the main document
        ...
        if draft_cips.any?
          Rails.logger.info "HAS DRAFT CIPS"
          ...
          Rails.logger.info "NO DRAFT CIPS FOR THIS ORGANISATION"
        end
        ...
        Rails.logger.info "Completed #{completed} of #{total}"
        Rails.logger.info "Failed #{failed} of #{total}"

        ## TODO: Delete the non-editionable worldwide organisation and all it's associations (a decision needs to be made on when we do this)
        ## TODO: Lock the old CIPs (can only be done after https://trello.com/c/McsLGp7i)
      end

      ## Uncomment the following line to run the migration and integrity without committing anything
