...
Then(/^I should see the promotional feature on the organisation's page$/) do
  ...
  expect(current_url).to eq(pathadmin_organisation_promotional_feature_url(@executive_office, promotional_feature))
  ...
  within record_css_selector(promotional_feature) do
    ...
    within record_css_selector(item) do
      expect(page).to have_content(item.summary)
                  .and have_link(item.title, href: item.title_url)
                  .and have_selector("img[src='#{item.image.s300.url}'][alt='#{item.image_alt_text}']")
    end
  end
end
...
Then(/^I should not be able to add any further feature items$/) do
  expect(page).to have_link("Add feature item")
end
