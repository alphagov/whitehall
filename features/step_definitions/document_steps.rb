...
When /^I view the (publication|policy|news article|consultation|speech|document) "([^"]*)"$/ do |_, title|
  click_link title
end
...
When /^I save my changes to the (publication|policy|news article|consultation|speech)$/ do |_|
  click_button "Save"
end

When /^I edit the (publication|policy|news article|consultation) changing the title to "([^"]*)"$/ do |_, new_title|
...
Then /^I should see the conflict between the (publication|policy|news article|consultation|speech) titles "([^"]*)" and "([^"]*)"$/ do |_, new_title, latest_title|
