...
Then(/^I should see that the publication has attachments$/) do
  ...
  expect(3).to eq(@edition.attachments.count)

  expect("Two pages title").to eq(@edition.attachments[1].title)
  expect("two-pages.pdf").to eq(@edition.attachments[1].filename)
  expect("Greenpaper title").to eq(@edition.attachments[2].title)
  expect("greenpaper.pdf").to eq(@edition.attachments[2].filename)
end
