...
Then(/^I (should|should not) see a preview to the ministers index page$/) do |should_or_should_not|
  selector = ".govuk-details__summary-text"
  text = "Share ministers page preview"
  if should_or_should_not == "should"
    expect(page).to have_selector(selector, text:)
  else
    expect(page).to_not have_selector(selector, text:)
  end
end
